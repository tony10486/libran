...
fn test_acs_journal() {
    ...
    assert!(out.contains("DOI: 10.1234/test"), "acs doi uppercase: {out}");
}
...
fn test_acs_book() {
    ...
    assert!(out.contains("Machine Learning Basics."), "acs book title: {out}");
...
fn test_acs_conference() {
    ...
    let out = render_ref(&doc, CitationStyle::AcsGuide2022, DisplayMode::InText);
    assert!(out.contains("Brown, A.; Wilson, B.;"), "acs conf authors: {out}");
...
fn test_ama_journal() {
    ...
    assert!(out.contains("Smith J, Lee J."), "ama authors no periods: {out}");
    assert!(out.contains("2023;42(3):123-145."), "ama year;vol(issue):pages: {out}");
    assert!(out.contains("doi: 10.1234/test"), "ama lowercase doi: {out}");
}
...
fn test_ama_book() {
    ...
    assert!(out.contains("Machine Learning Basics."), "ama book title: {out}");
...
fn test_ama_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::Ama11th, DisplayMode::InText);
    assert!(out.contains("Brown A, Wilson B."), "ama conf authors: {out}");
...
fn test_apa_journal() {
    ...
    assert!(out.contains("Smith, J., & Lee, J."), "apa authors with ampersand: {out}");
    assert!(out.contains("(2023)."), "apa year in parens: {out}");
    assert!(out.contains("https://doi.org/10.1234/test"), "apa doi url: {out}");
}
...
fn test_apa_book() {
    ...
    assert!(out.contains("Machine Learning Basics."), "apa book title: {out}");
}
...
fn test_apa_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::Apa7th, DisplayMode::InText);
    assert!(out.contains("Brown, A., & Wilson, B."), "apa conf authors: {out}");
    assert!(out.contains("NeurIPS 2023."), "apa conf name: {out}");
    assert!(out.contains("https://doi.org/10.4567/conf"), "apa conf doi url: {out}");
}
...
fn test_apsa_journal() {
    ...
    assert!(out.contains("Smith, John, and Jane Lee."), "apsa full names: {out}");
    assert!(out.contains("\"Deep Learning for NLP.\""), "apsa title in quotes: {out}");
    assert!(out.contains("42 (3): 123-145."), "apsa vol with spaces: {out}");
}
...
fn test_apsa_book() {
    ...
    assert!(out.contains("\"Machine Learning Basics.\""), "apsa book title: {out}");
...
fn test_apsa_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::Apsa2018, DisplayMode::InText);
    assert!(out.contains("Brown, Alice, and Bob Wilson."), "apsa conf authors: {out}");
    assert!(out.contains("\"Advanced Neural Networks.\""), "apsa conf title: {out}");
...
fn test_apsa_in_text() {
    ...
    assert!(out.contains("Smith, and Lee 2023"), "apsa in-text author-date: {out}");
...
fn test_asa_journal() {
    ...
    assert!(out.contains("Smith, John, and Jane Lee."), "asa full names: {out}");
    assert!(out.contains("42(3):123-145."), "asa vol no spaces: {out}");
    assert!(out.contains("\"Deep Learning for NLP.\""), "asa title in quotes: {out}");
}
...
fn test_asa_book() {
    ...
    assert!(out.contains("\"Machine Learning Basics.\""), "asa book title: {out}");
...
fn test_asa_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::Asa6th7th, DisplayMode::InText);
    assert!(out.contains("Brown, Alice, and Bob Wilson."), "asa conf authors: {out}");
    assert!(out.contains("\"Advanced Neural Networks.\""), "asa conf title: {out}");
...
fn test_asa_in_text() {
    ...
    assert!(out.contains("Smith, and Lee 2023"), "asa in-text author-date: {out}");
...
fn test_chicago_ad_journal() {
    let doc = journal_doc();
    let out = render_ref(&doc, CitationStyle::Chicago18AuthorDate, DisplayMode::InText);
    assert!(out.contains("Smith, John, and Jane Lee."), "chicago ad authors: {out}");
    assert!(out.contains("42, no. 3 (2023): 123-145."), "chicago ad vol/issue: {out}");
    assert!(out.contains("https://doi.org/10.1234/test"), "chicago ad doi: {out}");
}
...
fn test_chicago_ad_book() {
    let doc = book_doc();
    let out = render_ref(&doc, CitationStyle::Chicago18AuthorDate, DisplayMode::InText);
    assert!(out.contains("Smith, John."), "chicago ad book author: {out}");
    assert!(out.contains("\"Machine Learning Basics.\""), "chicago ad book title: {out}");
...
fn test_chicago_ad_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::Chicago18AuthorDate, DisplayMode::InText);
    assert!(out.contains("Brown, Alice, and Bob Wilson."), "chicago ad conf authors: {out}");
    assert!(out.contains("\"Advanced Neural Networks.\""), "chicago ad conf title: {out}");
    assert!(out.contains("https://doi.org/10.4567/conf"), "chicago ad conf doi: {out}");
}
...
fn test_chicago_ad_in_text() {
    ...
    assert!(out.contains("Smith, and Lee 2023"), "chicago ad in-text: {out}");
...
fn test_chicago_nb_journal() {
    let doc = journal_doc();
    let out = render_ref(&doc, CitationStyle::Chicago18NotesBib, DisplayMode::Footnotes);
    assert!(out.starts_with("1. "), "chicago nb footnote prefix: {out}");
    assert!(out.contains("John Smith, and Jane Lee"), "chicago nb first-name-first: {out}");
    assert!(out.contains("\"Deep Learning for NLP\""), "chicago nb title: {out}");
    assert!(out.contains("42, no. 3 (2023): 123-145"), "chicago nb vol/issue: {out}");
}
...
fn test_chicago_nb_book() {
    ...
    assert!(out.contains("John Smith"), "chicago nb book first-name-first: {out}");
    assert!(out.contains("\"Machine Learning Basics\""), "chicago nb book title: {out}");
}
...
fn test_chicago_nb_conference() {
    ...
    assert!(out.contains("Alice Brown, and Bob Wilson"), "chicago nb conf first-name-first: {out}");
...
fn test_chicago_shortened_journal() {
    let doc = journal_doc();
    let out = render_ref(&doc, CitationStyle::Chicago18ShortenedNotesBib, DisplayMode::Footnotes);
    assert!(out.starts_with("1. "), "chicago shortened footnote prefix: {out}");
    assert!(out.contains("John Smith, and Jane Lee"), "chicago shortened first-name-first: {out}");
    assert!(out.contains("\"Deep Learning for NLP\""), "chicago shortened title: {out}");
    assert!(out.contains("42, no. 3 (2023): 123-145"), "chicago shortened vol/issue: {out}");
}
...
fn test_chicago_shortened_book() {
    let doc = book_doc();
    let out = render_ref(&doc, CitationStyle::Chicago18ShortenedNotesBib, DisplayMode::Footnotes);
    assert!(out.starts_with("1. "), "chicago shortened book footnote: {out}");
    assert!(out.contains("John Smith"), "chicago shortened book first-name-first: {out}");
    assert!(out.contains("\"Machine Learning Basics\""), "chicago shortened book title: {out}");
}
...
fn test_chicago_shortened_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::Chicago18ShortenedNotesBib, DisplayMode::Footnotes);
    assert!(out.starts_with("1. "), "chicago shortened conf footnote: {out}");
    assert!(out.contains("Alice Brown, and Bob Wilson"), "chicago shortened conf authors: {out}");
    assert!(out.contains("NeurIPS 2023"), "chicago shortened conf name: {out}");
}
...
fn test_ctr_harvard_journal() {
    let doc = journal_doc();
    let out = render_ref(&doc, CitationStyle::CiteThemRight12thHarvard, DisplayMode::InText);
    assert!(out.contains("Smith, J. and Lee, J."), "ctr harvard initials no space: {out}");
    assert!(out.contains("(2023) 'Deep Learning for NLP'"), "ctr harvard year+single-quote title: {out}");
    assert!(out.contains("42(3), pp. 123-145"), "ctr harvard vol/pages: {out}");
}
...
fn test_ctr_harvard_book() {
    ...
    assert!(out.contains("'Machine Learning Basics'"), "ctr harvard book single-quote title: {out}");
...
fn test_ctr_harvard_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::CiteThemRight12thHarvard, DisplayMode::InText);
    assert!(out.contains("Brown, A. and Wilson, B."), "ctr harvard conf authors: {out}");
    assert!(out.contains("'Advanced Neural Networks'"), "ctr harvard conf single-quote title: {out}");
...
fn test_ctr_harvard_in_text() {
    ...
    assert!(out.contains("Smith and Lee 2023"), "ctr harvard in-text: {out}");
    assert!(out.contains("p. 123-145"), "ctr harvard in-text page: {out}");
}
...
fn test_elsevier_harvard_journal() {
    let doc = journal_doc();
    let out = render_ref(&doc, CitationStyle::ElsevierHarvardWithTitles, DisplayMode::InText);
    assert!(out.contains("Smith, J., Lee, J.,"), "elsevier no 'and': {out}");
...
fn test_elsevier_harvard_book() {
    ...
    assert!(out.contains("Machine Learning Basics."), "elsevier book title: {out}");
...
fn test_elsevier_harvard_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::ElsevierHarvardWithTitles, DisplayMode::InText);
    assert!(out.contains("Brown, A., Wilson, B.,"), "elsevier conf authors no 'and': {out}");
    assert!(out.contains("Advanced Neural Networks."), "elsevier conf title: {out}");
...
fn test_elsevier_harvard_in_text() {
    ...
    assert!(out.contains("Smith and Lee 2023"), "elsevier in-text: {out}");
...
fn test_ieee_journal() {
    ...
    assert!(out.contains("J. Smith and J. Lee,"), "ieee initials first: {out}");
    assert!(out.contains("\"Deep Learning for NLP\","), "ieee title in quotes: {out}");
...
fn test_ieee_book() {
    ...
    assert!(out.contains("\"Machine Learning Basics\","), "ieee book title in quotes: {out}");
...
fn test_ieee_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::IeeeV11_29_2023, DisplayMode::InText);
    assert!(out.contains("A. Brown and B. Wilson,"), "ieee conf initials first: {out}");
    assert!(out.contains("\"Advanced Neural Networks\","), "ieee conf title: {out}");
...
fn test_mhra_journal() {
    ...
    assert!(out.contains("John Smith and Jane Lee"), "mhra first-name-first: {out}");
    assert!(out.contains("'Deep Learning for NLP'"), "mhra single-quote title: {out}");
    assert!(out.contains("XLII"), "mhra roman numeral volume: {out}");
}
...
fn test_mhra_book() {
    ...
    assert!(out.contains("John Smith"), "mhra book first-name-first: {out}");
    assert!(out.contains("'Machine Learning Basics'"), "mhra book single-quote title: {out}");
}
...
fn test_mhra_conference() {
    ...
    assert!(out.contains("Alice Brown and Bob Wilson"), "mhra conf first-name-first: {out}");
    assert!(out.contains("'Advanced Neural Networks'"), "mhra conf single-quote title: {out}");
}
...
fn test_mla_journal() {
    ...
    assert!(out.contains("Smith, John and Jane Lee."), "mla two authors full names: {out}");
    assert!(out.contains("\"Deep Learning for NLP.\""), "mla title in quotes: {out}");
...
fn test_mla_book() {
    ...
    assert!(out.contains("\"Machine Learning Basics.\""), "mla book title: {out}");
...
fn test_mla_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::Mla9thInText, DisplayMode::InText);
    assert!(out.contains("Brown, Alice and Bob Wilson."), "mla conf authors: {out}");
    assert!(out.contains("\"Advanced Neural Networks.\""), "mla conf title: {out}");
...
fn test_nature_journal() {
    ...
    assert!(out.contains("Smith, J. & Lee, J."), "nature authors with ampersand: {out}");
    assert!(out.contains("42, 123-145 (2023)"), "nature vol pages year: {out}");
...
fn test_nature_book() {
    ...
    assert!(out.contains("Machine Learning Basics."), "nature book title: {out}");
...
fn test_nature_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::Nature, DisplayMode::InText);
    assert!(out.contains("Brown, A. & Wilson, B."), "nature conf authors: {out}");
...
fn test_vancouver_journal() {
    let doc = journal_doc();
    let out = render_ref(&doc, CitationStyle::NlmVancouverCitingMedicine2nd, DisplayMode::InText);
    assert!(out.contains("Smith J, Lee J."), "vancouver authors no periods: {out}");
    assert!(out.contains("2023;42(3):123-145."), "vancouver year;vol(issue):pages: {out}");
...
fn test_vancouver_book() {
    ...
    assert!(out.contains("Machine Learning Basics."), "vancouver book title: {out}");
...
fn test_vancouver_conference() {
    let doc = conference_doc();
    let out = render_ref(&doc, CitationStyle::NlmVancouverCitingMedicine2nd, DisplayMode::InText);
    assert!(out.contains("Brown A, Wilson B."), "vancouver conf authors: {out}");
    assert!(out.contains("NeurIPS 2023."), "vancouver conf name: {out}");
    assert!(out.contains("doi: 10.4567/conf"), "vancouver conf doi: {out}");
}
