...
pub fn parse_csl_json(json: &str) -> Result<Vec<Document>> {
    ...
    let documents = items.into_iter().map(|item| {
...
mod tests {
    ...
    fn test_round_trip_bibtex_to_csl_to_bibtex() {
        ...
        assert!(bib2.contains("Smith, John"), "first author preserved: {bib2}");
        assert!(bib2.contains("Lee, Jane"), "second author preserved: {bib2}");
