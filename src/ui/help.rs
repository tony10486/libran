...
fn page_0_lines() -> Vec<Line<'static>> {
    vec![
        ...
        help_line("  *", "즐겨찾기 필터 토글 (별점 5점 문헌만 표시)"),
        ...
        help_line(
            "  w",
            "DOI/arXiv ID 일괄 가져오기 (한 줄에 하나, Enter 제출, 500ms 간격)",
        ),
        help_line(
            "  I",
            "BibTeX(.bib) 파일 가져오기 (경로 입력 → 자체 파서 → 일괄 등록)",
        ),
        help_line(
            "  F",
            "전방 인용 조회: 이 논문을 인용한 후속 논문 수 (OpenAlex, DOI 필요)",
        ),
        help_line(
            "  E",
            "저자 병합: 원본 저자명 → 정식 저자명 2단계 입력 (DB 일괄 교체)",
        ),
        help_line(
            "  b",
            "PDF 북마크/TOC 추출 (상세 보기에서, 페이지 번호와 함께 표시)",
        ),
        Line::from(""),
        sub_header("프로젝트 · 시리즈"),
...
fn page_1_lines() -> Vec<Line<'static>> {
    vec![
        ...
        help_line(
            "  실패 시",
            "클립보드 불가 → ~/.libran/clipboard.txt 자동 저장",
        ),
...
fn page_2_lines() -> Vec<Line<'static>> {
    vec![
        ...
        help_line(
            "  v",
            "현재 검색어를 이름과 함께 저장, 다시 불러와 적용 가능",
        ),
        help_line(
            "  i",
            "라이브러리 통계 대시보드 (총 문헌/읽기/연도/저자/저널)",
        ),
        help_line(
            "  b",
            "상세 보기에서 PDF 북마크/TOC 추출 (페이지 번호 포함)",
        ),
        help_line(
            "  w",
            "DOI/arXiv ID 여러 줄 일괄 가져오기 (CrossRef/arXiv API)",
        ),
        ...
        help_line(
            "  E",
            "저자 병합 — 원본 저자명을 정식 저자명으로 일괄 교체 (2단계 입력)",
        ),
        Line::from(""),
        page_footer(2),
    ]
...
fn page_footer(page: usize) -> Line<'static> {
    Line::from(vec![
        ...
        Span::raw("  "),
        Span::styled("Tab 다음 페이지", Style::default().fg(theme::dim())),
        Span::raw("  "),
