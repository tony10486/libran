...
fn render(f: &mut Frame, list_state: &mut ListState, active_tab: usize) {
    ...
    let bottom = Paragraph::new(Line::from(vec![
        Span::styled(" 1-4", Style::default().fg(Color::Cyan)), Span::raw(" 탭 "),
...
fn render_projects_tab(f: &mut Frame, area: ratatui::layout::Rect, list_state: &mut ListState) {
    let items: Vec<ListItem> = vec![
        ListItem::new(Line::from(vec![
            Span::styled("  머신러닝 가속 연구  ", Style::default().add_modifier(Modifier::BOLD)),
            Span::styled("(12)", Style::default().fg(Color::Yellow)),
        ])),
        ListItem::new("    2024-01 ~ 현재 · 다학제 연구"),
        ListItem::new(""),
        ListItem::new(Line::from(vec![
            Span::styled("  CUI 렌더러 설계  ", Style::default().add_modifier(Modifier::BOLD)),
            Span::styled("(7)", Style::default().fg(Color::Yellow)),
        ])),
        ListItem::new("    2024-03 ~ 현재 · 컴퓨터과학"),
        ListItem::new(""),
        ListItem::new(Line::from(vec![
            Span::styled("  수학적 모델링  ", Style::default().add_modifier(Modifier::BOLD)),
    ...
    let list = List::default()
        .items(items)
        .block(Block::default().borders(Borders::ALL).title(" 프로젝트 (3) "))
        .highlight_style(Style::default().bg(Color::DarkGray).add_modifier(Modifier::BOLD));
    f.render_stateful_widget(list, area, list_state);
}

fn render_classification_tab(f: &mut Frame, area: ratatui::layout::Rect, list_state: &mut ListState) {
    let items: Vec<ListItem> = vec![
        ListItem::new(Line::from(vec![
            Span::styled(" UDC", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            Span::raw(" (메인) "),
        ...
        ListItem::new(""),
        ListItem::new(Line::from(vec![
            Span::styled(" MSC2020", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
            Span::raw(" (보조) "),
...
fn render_settings_tab(f: &mut Frame, area: ratatui::layout::Rect) {
    let settings = Paragraph::new(vec![
        Line::from(""),
        Line::from(vec![Span::styled("  API 모드        ", Style::default().fg(Color::Cyan)), Span::raw("오프라인")]),
        Line::from(""),
        Line::from(vec![Span::styled("  파일 보관       ", Style::default().fg(Color::Cyan)), Span::raw("라이브러리 복사")]),
        Line::from(""),
        Line::from(vec![Span::styled("  인용키 모드     ", Style::default().fg(Color::Cyan)), Span::raw("성+연도 (AuthorYear)")]),
        Line::from(""),
        Line::from(vec![Span::styled("  메인 분류       ", Style::default().fg(Color::Cyan)), Span::raw("UDC Summary")]),
        Line::from(""),
        Line::from(vec![Span::styled("  활성 스킴       ", Style::default().fg(Color::Cyan)), Span::raw("UDC, PhySH, MSC2020")]),
        Line::from(""),
        Line::from(vec![Span::styled("  라벨 언어       ", Style::default().fg(Color::Cyan)), Span::raw("English")]),
        Line::from(""),
        Line::from(vec![Span::styled("  DB 경로        ", Style::default().fg(Color::Cyan)), Span::raw("~/.libran/libran.db")]),
        Line::from(""),
        Line::from(""),
        Line::from(vec![Span::styled("  [e]", Style::default().fg(Color::Yellow)), Span::raw(" 편집  "), Span::styled("[Enter]", Style::default().fg(Color::Yellow)), Span::raw(" 확인")]),
    ])
